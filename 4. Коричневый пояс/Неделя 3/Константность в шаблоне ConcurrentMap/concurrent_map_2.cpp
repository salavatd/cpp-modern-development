...
public:
    ...
    explicit ConcurrentMap(size_t bucket_count) : mBuckets_(bucket_count) {}

    WriteAccess operator[](const K &key) {
        auto &bucket = mBuckets_[hasher(key) % mBuckets_.size()];
        return WriteAccess{lock_guard(bucket.m), bucket.values[key]};
    }

    ReadAccess At(const K &key) const {
        const auto &bucket = mBuckets_.at(hasher(key) % mBuckets_.size());
        return ReadAccess{lock_guard(bucket.m), bucket.values.at(key)};
    }

    bool Has(const K &key) const {
        return mBuckets_[hasher(key) % mBuckets_.size()].values.count(key);
    }

    MapType BuildOrdinaryMap() const {
        MapType result;
        for (auto &bucket : mBuckets_) {
            for (const auto&[key, value] : bucket.values) {
