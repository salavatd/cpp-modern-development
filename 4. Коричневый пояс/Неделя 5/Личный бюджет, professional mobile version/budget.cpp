...
public:
    ...
    void CombineWith(const BulkLinearUpdater &other) {
        tax_.count = tax_.count * other.tax_.count;
        add_.delta = add_.delta * other.tax_.count + other.add_.delta;
        spend_.delta += other.spend_.delta;
    }

    pair<double, double> Collapse(pair<double, double> origin, IndexSegment segment) const {
        return {
                origin.first * tax_.count + add_.delta * segment.length(),
                origin.second + spend_.delta * segment.length()
        };
    }
...
class BudgetManager : public SummingSegmentTree<pair<double, double>, BulkLinearUpdater> {
public:
...
struct ComputeIncomeRequest : ReadRequest<double> {
    ...
    double Process(const BudgetManager &manager) const override {
        pair<double, double> budget = manager.ComputeSum(MakeDateSegment(date_from, date_to));
...
struct PayTaxRequest : ModifyRequest {
    ...
    void Process(BudgetManager &manager) const override {
        const auto date_segment = MakeDateSegment(date_from, date_to);
        const double tax = 1.0 - percentage / 100.0;
        manager.AddBulkOperation(date_segment, BulkTaxApplier{tax});
    }
