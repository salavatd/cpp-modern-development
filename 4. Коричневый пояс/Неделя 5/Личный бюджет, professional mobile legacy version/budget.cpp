...
array<double, VERTEX_COUNT> tree_add_values, tree_spent_values, tree_add, tree_spent, tree_factor;

void Init() {
    tree_add_values.fill(0);
...
void Push(size_t v, size_t l, size_t r) {
    for (size_t w = v * 2; w <= v * 2 + 1; ++w) {
        if (w < VERTEX_COUNT) {
            ...
            (tree_add_values[w] *= tree_factor[v]) += tree_add[v] * (r - l) / 2;
...
double
ComputeSum(const array<double, VERTEX_COUNT> &tree_op_values, size_t v, size_t l, size_t r, size_t ql, size_t qr) {
    if (v >= VERTEX_COUNT || qr <= l || r <= ql) {
        return 0;
    }
    Push(v, l, r);
    if (ql <= l && r <= qr) {
        return tree_op_values[v];
    }
    return ComputeSum(tree_op_values, v * 2, l, (l + r) / 2, ql, qr)
           + ComputeSum(tree_op_values, v * 2 + 1, (l + r) / 2, r, ql, qr);
}

void
Add(array<double, VERTEX_COUNT> &tree_op_values, array<double, VERTEX_COUNT> &tree_op, size_t v, size_t l, size_t r,
    size_t ql, size_t qr, double value) {
    if (v >= VERTEX_COUNT || qr <= l || r <= ql) {
        return;
    }
    Push(v, l, r);
    if (ql <= l && r <= qr) {
        tree_op[v] += value;
        tree_op_values[v] += value * (r - l);
        return;
    }
    Add(tree_op_values, tree_op, v * 2, l, (l + r) / 2, ql, qr, value);
    Add(tree_op_values, tree_op, v * 2 + 1, (l + r) / 2, r, ql, qr, value);
    tree_op_values[v] =
            (v * 2 < VERTEX_COUNT ? tree_op_values[v * 2] : 0)
            + (v * 2 + 1 < VERTEX_COUNT ? tree_op_values[v * 2 + 1] : 0);
}
...
void Multiply(size_t v, size_t l, size_t r, size_t ql, size_t qr, size_t percentage) {
    ...
    double tax = 1 - percentage / 100.0;
    Push(v, l, r);
    if (ql <= l && r <= qr) {
        tree_factor[v] *= tax;
        tree_add[v] *= tax;
        tree_add_values[v] *= tax;
        return;
    }
    Multiply(v * 2, l, (l + r) / 2, ql, qr, percentage);
    Multiply(v * 2 + 1, (l + r) / 2, r, ql, qr, percentage);
    tree_add_values[v] =
            (v * 2 < VERTEX_COUNT ? tree_add_values[v * 2] : 0)
            + (v * 2 + 1 < VERTEX_COUNT ? tree_add_values[v * 2 + 1] : 0);
}
...
int main() {
    ...
    for (int i = 0; i < q; ++i) {
        ...
        if (query_type == "ComputeIncome") {
            cout << ComputeSum(tree_add_values, 1, 0, DAY_COUNT_P2, idx_from, idx_to)
                    - ComputeSum(tree_spent_values, 1, 0, DAY_COUNT_P2, idx_from, idx_to) << endl;
            ...
            double value;
            cin >> value;
            Add(tree_add_values, tree_add, 1, 0, DAY_COUNT_P2, idx_from, idx_to, value / (idx_to - idx_from));
        } else if (query_type == "Spend") {
            double value;
            cin >> value;
            Add(tree_spent_values, tree_spent, 1, 0, DAY_COUNT_P2, idx_from, idx_to, value / (idx_to - idx_from));
        }
    }
