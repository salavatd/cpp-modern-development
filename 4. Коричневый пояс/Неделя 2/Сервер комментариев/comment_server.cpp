...
class HttpResponse {
    HttpCode mCode;
    vector<HttpHeader> mHeaders;
    string mContent;
public:
    explicit HttpResponse(HttpCode code) : mCode(code) {}

    HttpResponse &AddHeader(string name, string value) {
        mHeaders.push_back({move(name), move(value)});
        return *this;
    }

    HttpResponse &SetContent(string a_content) {
        mContent = move(a_content);
        return *this;
    }
    ...
    friend ostream &operator<<(ostream &output, const HttpResponse &resp) {
        ...
        output << '\n';
        for (const auto &header : resp.mHeaders) {
            output << header.name << ": " << header.value << '\n';
        }
        if (!resp.mContent.empty()) {
            output << "Content-Length: " << resp.mContent.size() << "\n\n";
            output << resp.mContent;
        } else {
            output << '\n';
        }
        return output;
