...
class AirportCounter {
    const static size_t mAirportsCount = static_cast<size_t>(TAirport::Last_);

    array<size_t, mAirportsCount> mAirports;

public:
    // конструктор по умолчанию: список элементов пока пуст
    AirportCounter() : mAirports() {}

    // конструктор от диапазона элементов типа TAirport
    template<typename TIterator>
    AirportCounter(TIterator begin, TIterator end) : mAirports() {
        for (auto tIterator = begin; tIterator != end; tIterator++) {
            mAirports[size_t(*tIterator)]++;
        }
    }

    // получить количество элементов, равных данному
    size_t Get(TAirport airport) const {
        return mAirports.at(size_t(airport));
    }

    // добавить данный элемент
    void Insert(TAirport airport) {
        mAirports[size_t(airport)]++;
    }

    // удалить одно вхождение данного элемента
    void EraseOne(TAirport airport) {
        mAirports[size_t(airport)] = mAirports[size_t(airport)] ? mAirports[size_t(airport)] - 1 : 0;
    }

    // удалить все вхождения данного элемента
    void EraseAll(TAirport airport) {
        mAirports[size_t(airport)] = 0;
    }

    using Item = pair<TAirport, size_t>;
    using Items = array<Item, mAirportsCount>;

    // получить некоторый объект, по которому можно проитерироваться,
    // получив набор объектов типа Item - пар (аэропорт, количество),
    // упорядоченных по аэропорту
    Items GetItems() const {
        array<Item, mAirportsCount> result;
        for (size_t i = 0; i < mAirportsCount; i++) {
            result[i] = make_pair(TAirport(i), mAirports.at(i));
        }
        return result;
    }
};
